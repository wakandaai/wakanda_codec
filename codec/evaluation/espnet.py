...
# =============================================================================
# ESPNet PESQ Implementation
# Source: https://github.com/espnet/espnet/blob/master/utils/eval-source-separation.py
# Copyright ESPNet Contributors  
# =============================================================================

def _espnet_eval_PESQ(ref, enh, fs, compute_permutation: bool = True, wideband: bool = True):
    """Evaluate PESQ
    ...
    :param ref (np.ndarray): Reference (Nsrc, Nframe, Nmic)
    :param enh (np.ndarray): Enhanced (Nsrc, Nframe, Nmic)
    :param fs (int): Sample frequency
    :param compute_permutation (bool):
...
def compute_pesq(reference: Union[str, np.ndarray],
                decoded: Union[str, np.ndarray], 
    ...
    Args:
        ...
        decoded: Decoded audio (file path or numpy array)
        sample_rate: Sample rate in Hz (required if using numpy arrays)
        wideband: Use wideband PESQ if True, narrowband if False
        
    Returns:
        PESQ score (higher is better, range -0.5 to 4.5)
        
    Note:
        Requires PESQ binary installed and sample rate of 8kHz or 16kHz
        
    Example:
        >>> pesq_score = compute_pesq("reference.wav", "decoded.wav")
        >>> pesq_nb = compute_pesq(ref_array, dec_array, 8000, wideband=False)
    """
    ref_audio, dec_audio, sr = _validate_inputs(reference, decoded, sample_rate)

    if sr not in (8000, 16000):
        raise ValueError(f"PESQ requires 8kHz or 16kHz sample rate, got {sr}Hz")
    
    # Convert to ESPNet format  
    ref_audio = _prepare_audio_for_espnet(ref_audio)
    dec_audio = _prepare_audio_for_espnet(dec_audio)

    # Call ESPNet function
    pesq_scores, _ = _espnet_eval_PESQ(
        ref_audio, dec_audio, sr,
        compute_permutation=False,
        wideband=wideband
    )
